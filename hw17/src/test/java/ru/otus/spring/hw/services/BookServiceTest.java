...
import ru.otus.spring.hw.repositories.*;
...
class BookServiceTest {
    ...
    void findById_NotFound() {
        ...
        Mockito.when(bookRepository.findById(nonExistentBookId)).thenReturn(Optional.empty());

        Assertions.assertThatThrownBy(() -> bookService.findById(nonExistentBookId))
                .isInstanceOf(EntityNotFoundException.class)
                .hasMessage("Book with id %d not found".formatted(nonExistentBookId));

        Mockito.verify(bookRepository, Mockito.times(1)).findById(nonExistentBookId);
    }
