...
class CommentServiceTest {
    ...
    @DisplayName("should throw EntityNotFoundException when comment not found by id")
    @Test
    void findById_NotFound() {
        long nonExistentCommentId = 999L;
        Mockito.when(commentRepository.findById(nonExistentCommentId)).thenReturn(Optional.empty());

        Assertions.assertThatThrownBy(() -> commentService.findById(nonExistentCommentId))
                .isInstanceOf(EntityNotFoundException.class)
                .hasMessage("Comment with id %d not found".formatted(nonExistentCommentId));

        Mockito.verify(commentRepository, Mockito.times(1)).findById(nonExistentCommentId);
    }
